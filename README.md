...
- The model runs in float32. GO ranking and thresholded term sets are effectively identical across CPU/GPU. Differences in probabilities are of order  $\mathcal{O}(10^{-7})$ between GPUs (the same CUDA versions) and $\mathcal{O}(10^{-6})$ between GPU and CPU.
