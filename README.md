...
- The model runs in float32. GO ranking and thresholded term sets are effectively identical across CPU/GPU and cuDNN versions, but probabilities differ slightly: $\mathcal{O}(10^{-3}-10^{-4})$ on structure, $\mathcal{O}(10^{-4}-10^{-5})$ on fusion, $\mathcal{O}(10^{-6})$ on sequence. The main cause is **TF32**: on the GPU, cuDNN runs the structure model's convolutions in reduced-precision TF32 by default (`torch.backends.cudnn.allow_tf32=True`).
